...
class DataWriter():
    ...
    def write(self, timestamp, obs, supervision, reward, control_diff=None):
        ...
        if 'bbox' in obs.keys():
            data_dict['obs']['bbox'] = obs['bbox']
        
        if 'lanes' in obs.keys():
            data_dict['obs']['lanes'] = obs['lanes']
        ...
        if 'bbox' in obs.keys():
            num_boxes = np.sum(data_dict['obs']['bbox']['classes'] > 0)
        else:
            num_boxes = "detection disabled"
        txt_1 = f'{action_str} spd:{speed[0]:5.2f} bboxes:{num_boxes}'
        render_rgb = cv2.putText(render_rgb, txt_1, (0, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)

        if 'lanes' in obs.keys():
            lane_obs = obs['lanes']['lanes']
            lane_vis = np.dstack((lane_obs * 255.0, np.zeros_like(lane_obs), np.zeros_like(lane_obs))).astype(np.uint8)
            render_rgb = np.hstack((render_rgb, lane_vis))
        return render_rgb
        ...
        if self._data_list[0]['control_diff'] is not None:
            ...
            if remove_final_steps:
                if terminal_debug['traffic_rule_violated']:
                    ...
                    del self._data_list[-step_to_delete:]
                    if len(self._data_list) < 100: # 300
                        valid = False
