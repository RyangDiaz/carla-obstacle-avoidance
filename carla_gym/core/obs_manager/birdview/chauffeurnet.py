...
class ObsManager(ObsManagerBase):
    ...
    def get_observation(self):
        ...
        vehicle_bbox_list += self._world.get_level_bbs(carla.CityObjectLabel.Bus)
        vehicle_bbox_list += self._world.get_level_bbs(carla.CityObjectLabel.Truck)
        vehicle_bbox_list += self._world.get_level_bbs(carla.CityObjectLabel.Motorcycle)
    ...
    def _get_mask_from_stopline_vtx(self, stopline_vtx, M_warp):
        ...
        for sp_locs in stopline_vtx:
            ...
            cv.line(mask, tuple([int(a) for a in stopline_warped[0,0]]), tuple(int(b) for b in stopline_warped[1,0]),
                    color=1, thickness=6)
