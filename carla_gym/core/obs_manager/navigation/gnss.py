...
class ObsManager(ObsManagerBase):
    ...
    def _define_obs_space(self):
        ...
        self.obs_space = spaces.Dict({
            ...
            'command': spaces.Box(low=-1, high=8, shape=(1,), dtype=np.int8)
        })
    ...
    def get_observation(self):
        ...
        # If vehicle in avoid obstacles or return to road modes, switch to those branches
        if self._parent_actor.avoid_obstacles:
            command = np.array([7], dtype=np.int8)
        elif self._parent_actor.return_to_road:
            command = np.array([8], dtype=np.int8)
        else:
