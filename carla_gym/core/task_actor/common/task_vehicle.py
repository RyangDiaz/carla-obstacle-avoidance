...
class TaskVehicle(object):
    ...
    def __init__(self, vehicle, target_transforms, spawn_transforms, endless):
        ...
        self._temp_last_route_location = None
    ...
    def _truncate_global_and_temp_route_till_local_target(self, windows_size=5):
        ev_location = self.vehicle.get_location()
        closest_idx_avoid = 0
        ...
        for i in range(len(self._global_route)-1):
            ...
            if dot_ve_wp > 0:
                closest_idx_avoid = i+1
        
        closest_idx = 0

        for i in range(len(self._temp_global_route)-1):
            if i > windows_size:
                break
            
            loc0 = self._temp_global_route[i][0].transform.location
            loc1 = self._temp_global_route[i+1][0].transform.location
        ...
        distance_traveled = self._compute_route_length(self._temp_global_route[:closest_idx_avoid+1])
        self._route_completed += distance_traveled

        if closest_idx > 0:
            self._last_route_location = carla.Location(self._temp_global_route[0][0].transform.location)

        self._global_route = self._global_route[closest_idx_avoid:]
        self._temp_global_route = self._temp_global_route[closest_idx:]
        return distance_traveled
    ...
    def _check_avoid_and_return(self, timestamp):
        ...
        # TODO: Perform hazard vehicle check with respect to waypoints rather than vehicle
        hazard_vehicle_loc = lbc_hazard_vehicle(obs_vehicle, proximity_threshold=12.0, up_angle_th=6)
        hazard_ped_loc = lbc_hazard_walker(obs_pedestrian, proximity_threshold=12.0)
        ...
        if self.return_to_road:
            ...
            if self._temp_global_route is not None:
                ...
                # Check back on route for return to road
                if lateral_distance < 0.25 and angle_difference < 0.05:
                    self.return_to_road = False

                    # Correct to true global route
                    print("[DEBUG] Returned to route!")
                    self._global_route = self._temp_global_route
                    assert self._global_route is not None
                    self._temp_global_route = None

    
    def _handle_avoid(self):
        print("[DEBUG] Handling block")

        if len(self._global_route) < 20:
            print("[DEBUG] Route cannot be finished due to block")
            return
        
        wp_ahead = self._global_route[12][0] # Turning point
        wp_ahead_forward1 = self._global_route[20][0] # Maintain changed lane
        wp_ahead_forward2 = self._global_route[22][0]

        # Check for available lane change
        if wp_ahead.left_lane_marking.lane_change is not carla.LaneChange.NONE:
            lane_change = wp_ahead.get_left_lane()
            lane_change_forward = wp_ahead_forward1.get_left_lane()
        elif wp_ahead.right_lane_marking.lane_change is not carla.LaneChange.NONE:
            lane_change = wp_ahead.get_right_lane()
            lane_change_forward = wp_ahead_forward1.get_right_lane()
        else:
            print('[DEBUG] No available lane change detected!')
            return
        
        if lane_change is None or lane_change_forward is None:
            print('[DEBUG] Lane change attempted, but was invalid!')
            return
        
        print('[DEBUG] Updating route with new lane change!')
        current_loc = self.vehicle.get_location()
        lc_loc = lane_change.transform.location
        lc_f1_loc = lane_change_forward.transform.location
        return_loc = wp_ahead_forward2.transform.location

        route_to_lane_change = self._planner.trace_route(current_loc, lc_loc)
        lane_change_to_forward = self._planner.trace_route(lc_loc, lc_f1_loc)
        forward_to_return = self._planner.trace_route(lc_f1_loc, return_loc)

        # Store original route for computing return to route state
        self._temp_global_route = self._global_route
        self._global_route = route_to_lane_change + lane_change_to_forward + forward_to_return + self._global_route[23:]
    
    def _handle_avoid_benchmark(self):
        self._temp_global_route = self._global_route
    ...
    def tick(self, timestamp):
        if self._temp_global_route is not None:
            distance_traveled = self._truncate_global_and_temp_route_till_local_target()
        else:
        ...
        if self.avoid_obstacles and self._temp_global_route is None:
            self._handle_avoid()                 # TODO: Comment this out during benchmarking
            # self._handle_avoid_benchmark()     # TODO: Uncomment this during benchmarking
    ...
    @property
    def temp_global_route(self):
        return self._temp_global_route
    ...
    # Gets route transform of true global route
    def get_route_transform(self):
        loc0 = self._last_route_location
        if self._temp_global_route is not None:
            route = self._temp_global_route
        else:
            route = self._global_route

        loc1 = route[0][0].transform.location

        if loc1.distance(loc0) < 0.1:
            yaw = route[0][0].transform.rotation.yaw
        else:
            f_vec = loc1 - loc0
