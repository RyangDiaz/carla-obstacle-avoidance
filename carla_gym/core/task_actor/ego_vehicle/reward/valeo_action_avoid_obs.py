...
'''
    Original attempt to learn obstacle-avoiding behavior from RL 
    To use this, change the filename to be valeo_action.py and rename the other valeo_action.py file
'''
...
class ValeoAction(object):
    ...
    def get(self, terminal_reward):
        ...
        # action
        if self._ego_vehicle.avoid_obstacles:
            # set desired steer based on lane change availability
            wp_ahead = self._ego_vehicle.global_route[8][0]
            if wp_ahead.left_lane_marking.lane_change is not carla.LaneChange.NONE:
                r_action = 1.0 - (np.abs(-1.0 - ev_control.steer)) # / 2.0
        ...
        if hazard_vehicle_loc is not None and not (self._ego_vehicle.avoid_obstacles or self._ego_vehicle.return_to_road):
        ...
        if hazard_ped_loc is not None and not (self._ego_vehicle.avoid_obstacles or self._ego_vehicle.return_to_road):
        ...
        oa = self._ego_vehicle.avoid_obstacles
        rtr = self._ego_vehicle.return_to_road
        tmp_glob = self._ego_vehicle.temp_global_route is not None
        glob_len = len(self._ego_vehicle.global_route)

        debug_texts = [
            f'ldist:{lateral_distance:5.2f} adif:{angle_difference:5.2f}',
            ...
            f'av_obs:{oa}, rtr:{rtr}, tmp:{tmp_glob}',
            f'glob:{glob_len}'
        ]
        reward_debug = {
