class LeaderboardDagger(object):
    ...
    # TODO: Remove done conditions for route deviation if vehicle is in av_obs or rtr
    def get(self, timestamp):
        ...
        c_route_compl = len(self._ego_vehicle.global_route) <= 1

        done = c_blocked or c_route_deviation or c_collision or c_run_rl or c_run_stop or timeout or c_route_compl
        ...
        if c_collision:
            print("[DEBUG] Illegal collision!")
        if c_run_rl:
            print("[DEBUG] Illegal red light running!")
        if c_run_stop:
            print("[DEBUG] Illegal stop sign running!")
        if c_route_compl:
            print("[DEBUG] Route completed!")
