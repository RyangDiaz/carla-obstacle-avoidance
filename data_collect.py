...
def collect_single(run_name, env, data_writer, driver_dict, driver_log_dir, coach_dict, coach_log_dir,
    ...
    while not done['__all__']:
        ...
        new_obs, reward, terminated, truncated, info = env.step(driver_control)
...
@ hydra.main(config_path='config', config_name='data_collect', version_base='1.1')
def main(cfg: DictConfig):
    ...
    for ev_id, ev_cfg in cfg.actors.items():
        ...
        if ev_cfg.coach is None:
            ...
            for k, v in OmegaConf.to_container(cfg.agent.cilrs_bbox.obs_configs).items():
    ...
    elif cfg.actors.hero.driver == 'cilrs_bbox':
        ckpt_project = cfg.agent.cilrs_bbox.wb_run_path.split('/')[1]
        ckpt_run_id = cfg.agent.cilrs_bbox.wb_run_path.split('/')[2]
        dataset_name = f'{ckpt_project}/{ckpt_run_id}'
        cfg.wb_project = ckpt_project
        wb_run_name = f'{dataset_root.name}/{dataset_name}'
        im_stack_idx = driver_dict[cfg.ev_id]._env_wrapper.im_stack_idx
    ...
    env = gym.make(env_setup['env_id'], obs_configs=obs_configs, reward_configs=reward_configs,
                   ...
                   seed=cfg.seed, no_rendering=cfg.no_rendering, render_mode='rgb_array', **env_setup['env_configs'])
    ...
    for task_idx in range(ckpt_task_idx, n_episodes_per_env):
        ...
        if cfg.log_video:
            ...
            if cfg.actors.hero.driver != 'cilrs' and cfg.actors.hero.driver != 'cilrs_bbox':
