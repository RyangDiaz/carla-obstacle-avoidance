...
'''
    Main RL expert agent for Roach that uses ground-truth bird's eye view images as input
'''
...
class RlBirdviewAgent():
    ...
    def setup(self, path_to_conf_file):
        ...
        if cfg.wb_run_path is not None and cfg.wb_run_path != '1':
            api = wandb.Api()
            ...
            try:
                ...
                # Prevent program from constantly restarting if wandb ckpt not found
                self._logger.info(f'Fatal error (wandb ckpt file not found). Exiting program...')
                sys.exit(0)
    ...
    @obs_configs.setter
    def obs_configs(self, value):
        self._obs_configs = value
