...
class CoILICRA(nn.Module):
    ...
    def __init__(self, im_shape, input_states, acc_as_action,
                 ...
                 lanenet_output_neurons=512,
                 measurements_neurons=[128, 128],
                 measurements_dropouts=[0.0, 0.0],
                 bbox_neurons=[128, 128],
                 bbox_dropouts=[0.0, 0.0],
                 join_neurons=[512],
                 join_dropouts=[0.0],
        ...
        # lanenet block
        self.lanenet = CoILICRA._load_lanenet_model(output_dim=lanenet_output_neurons)
        ...
        # bbox block
        self.bboxes = FC(params={'neurons': [50] + bbox_neurons, # 10 detections * (4 dims + 1 class)
                                 'dropouts': bbox_dropouts,
                                 'end_layer': nn.ReLU})

        # concat/join block
        self.join = Join(params={'after_process':
                                 FC(params={'neurons':
                                            [measurements_neurons[-1] + lanenet_output_neurons + bbox_neurons[-1]] + join_neurons,
    ...
    def forward(self, im, state, bboxes, lanes):
        if not self._video_input:
            b, c, t, h, w = im.shape
            im = im.view(b, c*t, h, w)

        # NOTE: We are not using raw RGB image observations in our model, only the outputs from the LaneNet and YOLO models.
        #       However, we are leaving in lines related to processing the raw image input in case one does want to include 
        #       RGB inputs. If you are doing so, comment out the below line and please be sure to modify the 'join' module in 
        #       cilrs_bbox/models/networks/join.py to accept your desired input modalities.
        im = th.zeros_like(im)

        """ ###### APPLY THE PERCEPTION MODULE """
        x = self.perception(im)

        """ ###### APPLY THE LANENET MODULE """
        l = self.lanenet(lanes)

        """ ###### APPLY THE MEASUREMENT MODULE """
        m = self.measurements(state)

        """ ###### APPLY THE BBOX MODULE """
        b = self.bboxes(bboxes)

        """ Join measurements, perception (LaneNet), and bboxes"""
        j = self.join(l, m, b)
    ...
    def forward_branch(self, command, im, state, bboxes, lanes):
        with th.no_grad():
            im_tensor = im.unsqueeze(0).to(self.device)
            state_tensor = state.unsqueeze(0).to(self.device)
            bboxes_tensor = bboxes.flatten().unsqueeze(0).to(self.device) # Flatten bbox coordinates and classes
            lanes_tensor = lanes.unsqueeze(0).to(self.device)
            command_tensor = command.to(self.device)
            command_tensor.clamp_(0, self.number_of_branches-1)
            outputs = self.forward(im_tensor, state_tensor, bboxes_tensor, lanes_tensor)
    ...
    @staticmethod
    def _load_lanenet_model(output_dim=512):
