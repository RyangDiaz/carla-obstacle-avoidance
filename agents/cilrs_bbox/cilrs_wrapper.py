...
COLOR_PINK = (239, 6, 215)
COLOR_TURQUOISE = (6, 239, 231)

'''
    Wrapper for visualizing IL agent behavior
'''
...
class CilrsWrapper():
    ...
    # TODO: Add in additional obstacle avoidance / lane return commands
    # TODO: Process image obs with LaneNet to get lane detections
    def process_obs(self, obs):
        ...
        assert command in [0, 1, 2, 3, 4, 5, 6, 7]
        ...
        bboxes = obs['bbox']['bboxes'].astype(np.float32)
        classes = np.expand_dims(obs['bbox']['classes'],axis=1).astype(np.float32)
        bboxes_all = np.hstack((bboxes, classes)).flatten()

        lanes = obs['lanes']['lanes'].astype(np.float32)
        lanes = cv2.resize(lanes, (128,128))
        lanes = np.expand_dims(lanes, axis=0)
        ...
        policy_input = {
            ...
            'bboxes': th.from_numpy(bboxes_all),
            'lanes': th.from_numpy(lanes)
        }
    ...
    def im_render(render_dict):
        ...
        lane_obs = render_dict['lanes']
        lane_vis = np.dstack((lane_obs * 255.0, np.zeros_like(lane_obs), np.zeros_like(lane_obs))).astype(np.uint8)
        # render_rgb = np.hstack((im, lane_vis))
        return im
